...
| 設定 | 値 | 対象 | 効果 |
|---|---|---|---|
| Experimental Conn | R・L両側で無効 | ― | `CONFIG_ZMK_BLE_EXPERIMENTAL_CONN` は 2026-05-31 に撤去済み。なお ZMK 上流での唯一の効果は 2M PHY の既定無効化 (`app/Kconfig`: `config BT_CTLR_PHY_2M / default n if ZMK_BLE_EXPERIMENTAL_CONN`) であり、`CONFIG_BT_CTLR_PHY_2M`（下記）はいずれにせよ既定でyのため、撤去の前後で実際のビルド結果は不変だった |
| NFCT_PINS_AS_GPIOS | 有効 | R・L両側 | NFC無線とBLEの干渉防止（安定版2つともあり） |
| BT_CTLR_PHY_2M / BT_AUTO_PHY_UPDATE | 未設定（Zephyr既定のyのまま） | R・L両側 | 〈Redundant Sigil Cleanup〉（2026-09-14）。原型が使う古いZephyr `v3.5.0+zmk-fixes` でも `BT_CTLR_PHY_2M` の既定は同じく y と実機ソースで確認済み ―― 明示指定は原型との差分ではなく実効果ゼロと判明したため、可読性目的でコメントアウトのみ実施（容量・挙動は完全に不変）。2台目以降ペアリング不調の原因調査では対象外と結論 |
| BT_GAP_AUTO_UPDATE_CONN_PARAMS | 未設定（Zephyr既定の有効のまま） | R・L両側 | 接続後に自動パラメータ再交渉。既定が既に有効のため明示指定を撤去（無変化） |
| BT_CONN_PARAM_UPDATE_TIMEOUT | 未設定（Zephyr既定5000ms） | R・L両側 | 〈Conn Param Timeout Sigil Removal〉（2026-09-14）。従来1000ms（接続から1秒後にパラメータ更新要求）はBluetooth Core仕様「Peripheralは接続確立後5秒間は接続パラメータ更新手続きを行うべきではない」に違反。出典元のZMK本家Issue #3381（sub-7.5ms低遅延スプリットリンク向けレシピ）は投稿者自身が「Bluetoothスペック非準拠、Zephyrデバイス間のみで機能する」と明記しており、ホスト向け接続への転用は不適切だった疑い。新規ペアリング（2台目以降）不調の切り分けのため既定5秒へ委ねる |
| BT_CTLR_DATA_LENGTH_MAX / BT_BUF_ACL_RX・TX_SIZE | 未設定（Zephyr既定27） | R・L両側 | 〈Data Length Sigil Removal〉（2026-09-14）。従来251はZephyr本家Issue #38782で「Windows/タブレット/非SnapdragonスマホでBT_HCI_ERR_CONN_TIMEOUTを伴うペアリング失敗を招く既知の互換性問題」と判明した値とほぼ同一。原型Cygnus-S-Lkeymouseにも存在せず既定へ委ねる |
...
| BT_PERIPHERAL_PREF_LATENCY（ホスト接続） | 未設定（Zephyr既定30、R・L両側） | R側（Central、ホスト接続）／L側（split link） | ホストとの接続でデータ無しの接続イベントを何回スキップしてよいか。従来 0（毎接続イベント必須応答）だったが、〈Idle Tax Removal〉（2026-09-12）で設定ごと撤去し既定30へ委ねた。0だとホストのBLEスタックがアイドル時も休めず常時稼働し、PC側が他処理で混み合うとキー入力・トラックボール移動が両方遅延する症状の主要容疑だったが、撤去により実機で解消確認済み（アイドルからの復帰時の初動遅延等の新たな副作用も無し）。原型 Cygnus-S-Lkeymouse にはこの設定自体が存在せず既定30のまま。**当時のSYSTEM LOGはL側も同時撤去済みと記録していたが、実際にはL側に0が残置されたままだった非対称を2026-09-16〈Idle Tax Removal · Bilateral Sync〉で是正**（L側はsplit link向けの設定でホスト接続とは無関係だが、「左右完全一致」の方針に合わせ撤去）。左右分割リンク側の `ZMK_SPLIT_BLE_PREF_LATENCY`（Split BLE Latency 行、下記）とは別物で変更していない |
...
| Insomnia pingInterval | **実験的に無効化**（従来3秒、R・L両側） | R・L両側 | 〈Wakeful Vigil Suspension〉（2026-09-13）。バッテリー%変動疑惑への切り分けのためコメントアウト。無効化により端末が実際にIDLE化するようになり、`battery.c`のバッテリー再測定タイマーがIDLE中は停止する既定挙動に戻る。BLE切断が再発したら復帰 |
| ZMK_SLEEP（Deep Sleep） | **無効化**（従来有効、R・L両側） | R・L両側 | 〈Deep Sleep Sigil Removal〉（2026-09-14）。ZMK本家Issue #3195「Central side does not wake from sleep when ZMK Studio is enabled (Zephyr 4.1)」の既知バグ条件（`CONFIG_ZMK_STUDIO=y`+`CONFIG_ZMK_SLEEP=y`、本構成が固定するZephyrはちょうど4.1.0）に一致していたため撤去。原型Cygnus-S-Lkeymouseも`CONFIG_ZMK_SLEEP`自体を設定せず既定（Deep Sleep無効）のまま。Studioの利便性を維持しつつ既知バグを回避、長時間放置時の追加省電力（Deep Sleep分）は失うがIDLE（5分、軽量スリープ）は維持 |
...
| PMW3610 force-awake | **再有効化**（2026-09-13〜09-16は無効化） | 〈Wakeful Vigil Suspension〉（2026-09-13）でバッテリー%変動切り分けのため無効化していたが、〈Silent Vigil Recall〉（2026-09-16）でBLE再接続直後にトラックボールのみ無反応になる症状の切り分け実験として再有効化。センサーを常時RUNモード（高消費電流）に固定し、REST復帰待ちが要因である可能性を一変数で検証中。実機シリアルログが採取できないため原因の確定はできておらず、効果がなければ再度無効化に戻す想定 |
...
| 3号機 | Excalibur | Dark Repulser（1号機と共通） | `Elucidator_Excalibur` |
...
>
> **[ CARDINAL ]** `CONFIG_ZMK_KEYBOARD_NAME` には **16文字**の上限がある（`zmk/app/src/ble.c` の `BUILD_ASSERT` で強制、超過するとビルド時に `BLE device name is too long` で失敗する）。号機名を追加する際は必ず16文字以内に収めること（`Holy Sword Excalibur` は20文字で超過したため `Excalibur` に短縮した経緯あり）。
...
| DATE | ENTRY |
|---|---|
| 2026-09-18 | 〈Artifact True Name〉— `build.yaml` の成果物ファイル名（`artifact-name`）を剣の真名だけに統一。1号機central/peripheralに未設定だった`artifact-name`を明示追加（`Elucidator`/`Dark_Repulser`）、2号機・3号機の`Elucidator_NightSkySword`/`Elucidator_Excalibur`から冗長な`Elucidator_`接頭辞を除去し`NightSkySword`/`Excalibur`に短縮。シールド定義・Kconfig・`CONFIG_ZMK_KEYBOARD_NAME`（BLE表示名）は変更なし——成果物ファイル名の見た目のみの調整。 |
...
| 2026-09-14 | 〈Deep Sleep Sigil Removal · Studio Compatibility〉— ユーザー依頼「その他も全体を調査して」を受けたBLE以外の全体監査で、`CONFIG_ZMK_STUDIO=y`と`CONFIG_ZMK_SLEEP=y`の同時有効化がZMK本家Issue #3195「Central side does not wake from sleep when ZMK Studio is enabled (Zephyr 4.1)」の既知バグ条件と一致していると判明。本リポジトリが固定するZephyrコミット(`58a5874a`)の`VERSION`ファイルを実際に確認し、ちょうど`4.1.0`であることを確認済み——該当Issueが指す「Zephyr 4.1アップグレード後」の条件に合致する。症状はcentral側（Elucidator）が長時間放置でDeep Sleepへ入った後、キー入力で復帰しなくなるというもの（PM API変更が原因と推測、修正はv0.4.0マイルストーン待ちで完了未確認）。ユーザーはStudioを今後も使い続けたい意向のため、対処は「Studioを切る」ではなく「Deep Sleep自体を撤去する」を選択。原型`Cygnus-S-Lkeymouse`も`CONFIG_ZMK_SLEEP`自体を設定しておらず既定（Deep Sleep無効）のままだったため、この選択は原型の既定挙動にも合致する。`CONFIG_ZMK_SLEEP`/`CONFIG_ZMK_IDLE_SLEEP_TIMEOUT`をR・L両側でコメントアウトし撤去、`CONFIG_ZMK_IDLE_TIMEOUT`（5分、軽量IDLE）は維持。CHARACTER PARAMETERS（NERVE LINK STABILITY）に行を新設。長時間放置時の追加省電力（Deep Sleep分）を失うトレードオフあり、実機検証待ち。 |
| 2026-09-14 | 〈Redundant Sigil Cleanup〉— ユーザー依頼で `CONFIG_BT_CTLR_PHY_2M`/`CONFIG_BT_AUTO_PHY_UPDATE` を追加調査。原型 `Dist16384/Cygnus-S-Lkeymouse` が使う古いZephyr `v3.5.0+zmk-fixes`（ZMK `v0.3`経由）のコントローラKconfigを実際に取得し確認したところ、`BT_CTLR_PHY_2M`の既定値は当時から既に`y`と判明。現行Zephyr（本リポジトリ固定コミット`58a5874a`）でも`BT_AUTO_PHY_UPDATE`の既定は`y if !BT_USER_PHY_UPDATE`。すなわち両者とも**原型・現行を問わずZephyr既定で最初から有効**であり、本構成の明示`=y`は「原型との差分」ではなく実効果ゼロの冗長記述だったと確定（前回SYSTEM LOGで「未確定」としていた点への回答）。容量・挙動に一切影響しないため2台目以降ペアリング不調の容疑からは除外。誤解を招く記述のため可読性目的でコメントアウトのみ実施（`Elucidator.conf`/`Dark_Repulser.conf`）。CHARACTER PARAMETERS（NERVE LINK STABILITY）に行を新設・Experimental Conn行の記述も実態へ整合。 |
| 2026-09-14 | 〈Pairing Sigil Purification · Spec Compliance Retrial〉— ユーザー報告「2台目以降をペアリングするとき、うまくいかないことが多い」を発端に調査。**発見1（確定・前回既知）**: Bluetoothレイヤー下段の `bt_disc_0..4`（`&bt BT_SEL N` 直後に他4プロファイルへ `&bt BT_DISC` を連射）は、本リポジトリ自身が過去2回（2026-05-26/2026-05-28 SYSTEM LOG）実機で確認・修正済みの「新規ペアリング中のBT_DISCが host securing flow とレースし bond 確立を破壊する」不具合の再発生形。`e77d38f`（2026-05-29、50キー版Administratorからの同期）で純粋`BT_SEL`化の修正が意図せず上書きされ復活していた。ユーザーは既に上段の純粋`&bt BT_SEL N`を使用しており本件は回避済みと確認、コード自体は未修正のまま残置（別途対応要）。**発見2（本コミットで対応）**: 原型 `Dist16384/Cygnus-S-Lkeymouse`（Mac複数プロファイルペアリング正常動作の報告あり）の `Cygnus_R/L.conf` と現行 `Elucidator/Dark_Repulser.conf` をBLE関連Kconfigで突き合わせたところ、原型には存在しない追加チューニング群を発見。うち `CONFIG_BT_CONN_PARAM_UPDATE_TIMEOUT=1000` は、Zephyr本家Kconfig（`subsys/bluetooth/host/Kconfig`）の既定値5000msが Bluetooth Core仕様 Core 4.2 Vol 3, Part C, 9.3.12.2「Peripheralは接続確立後5秒間は接続パラメータ更新手続きを行うべきではない」に準拠した値であるのに対し、明確な仕様違反値だったと判明。出典を辿ると ZMK本家Issue #3381（sub-7.5ms低遅延スプリットリンク向け実験的レシピ）に同一値の組み合わせ（`BT_GAP_AUTO_UPDATE_CONN_PARAMS=y` + `BT_CONN_PARAM_UPDATE_TIMEOUT=1000`）があり、投稿者自身が「Bluetoothスペック非準拠、Zephyrデバイス間のみで機能する」と明記 ―― 本来は左右スプリットリンク専用（本リポジトリには別枠の`CONFIG_ZMK_SPLIT_BLE_PREF_*`が既に存在）のレシピが、ホスト（Mac等）向け接続にも誤って適用されていた疑い。新規ペアリング中のSMPボンディングと接続1秒後の自動パラメータ更新要求が競合し失敗を招いていた可能性がある（bt_discと同型のレース機序）。加えて `CONFIG_BT_CTLR_DATA_LENGTH_MAX=251`（+`BT_BUF_ACL_RX/TX_SIZE=251`）も、Zephyr本家Issue #38782「`DATA_LENGTH_MAX=250`がWindows/タブレット/非SnapdragonスマホでBT_HCI_ERR_CONN_TIMEOUTを伴うペアリング失敗を招く既知の互換性問題」とほぼ同一値と判明。いずれも原型には存在せずZephyr既定に委ねられている。**未確定（今回のスコープ外）**: `CONFIG_BT_CTLR_PHY_2M`/`CONFIG_BT_AUTO_PHY_UPDATE`は同型の懸念があるものの外部一次情報での確証が取れず、Apple HID Interval Compat(`MIN_INT=6`/`MAX_INT=12`)やBT_MAX_CONN/PAIRED=6（5プロファイル運用に必須）は実機検証済みの意図的差分のため据え置き。**対応**: `CONFIG_BT_CONN_PARAM_UPDATE_TIMEOUT`・`CONFIG_BT_CTLR_DATA_LENGTH_MAX`・`CONFIG_BT_BUF_ACL_RX_SIZE`・`CONFIG_BT_BUF_ACL_TX_SIZE`（+冗長だった明示`CONFIG_BT_GAP_AUTO_UPDATE_CONN_PARAMS=y`、既定と同値のため撤去しても無変化）をR・L両側でコメントアウトしZephyr既定へ委ねる。CHARACTER PARAMETERS（NERVE LINK STABILITY）の該当行を実態へ更新。実機検証待ち。 |
| 2026-09-13 | 〈Wakeful Vigil Suspension · Dual Variable Experiment〉— Mac側バッテリー%の「実際より低い／高い」不一致、および利用中に「下がって、また戻る」変動があるという報告を受けて追加調査。前回（〈Peripheral Echo Silencing〉）で ZMK 本体のバッテリー計算・電圧測定ロジックが原型 Cygnus-S-Lkeymouse と完全一致することを確認済みだったため、今回は「電圧をどう測るか」ではなく「いつ測るか／測定時に何が電流を食っているか」の観点で調査。**発見1**: `Elucidator.overlay` の `force-awake;`（PMW3610をスリープさせず常時高消費電流のRUNモードに固定）は、原型 `Cygnus_R.conf` には存在せず既定の `CONFIG_PMW3610_FORCE_AWAKE=n`（無効）のまま。ユーザーからは「Cygnus使用時も起動遅延は気にならなかった」との追加報告があり、実機比較で必要性を検証しないまま予防的に追加された疑いがある（過去の過剰BLEチューニング群と同じ構図）。**発見2**: `CONFIG_ZMK_INSOMNIA_PING_ON_START=y`（3秒毎にダミーのバッテリーゲージセンサーイベントを発行し続けるBLE切断対策、原型には該当モジュール自体が存在しない）の実装を追跡したところ、このイベント自体は電流を消費しないが、ZMKの活動状態管理（`activity.c`）を常時ACTIVEに固定する副作用があり、`battery.c` のバッテリー再測定タイマー（既定60秒毎、`zmk_battery_start_reporting()`）がIDLE化で止まる既定挙動を無効化 ―― 端末が机上で放置されている間も24時間休みなく60秒毎の再測定が継続する。原型はIdleTimeout（5分、両リポジトリ共通）でこの再測定自体が止まるため、同じ瞬間的な電圧沈み込み（LiPoの内部抵抗によるIRドロップ、単発測定かつ長期平滑化なしの電圧分圧方式の既知の弱点）が発生しても観測頻度に差が出ると推定。**対応**: 両者を「1行コメントアウトで即復帰できる」形で実験的に無効化（`Elucidator.overlay` の `force-awake;`、`Elucidator.conf`/`Dark_Repulser.conf` 両側の `CONFIG_ZMK_INSOMNIA_PING_INTERVAL`/`_PING_ON_START`）。二変数同時実験のため、症状再発時にどちらが主犯かの切り分けは次段階の課題として残る。実機検証待ち・CHARACTER PARAMETERS（MOTION SENSOR CONFIG / NERVE LINK STABILITY）の該当行を実験中の値へ更新。**既知のリグレッションリスク**: force-awake無効化でトラックボール復帰時の初動遅延、Insomnia無効化でBLE切断（iPadOS等）が再発する可能性 ―― 発生時は該当行を戻すこと。 |
